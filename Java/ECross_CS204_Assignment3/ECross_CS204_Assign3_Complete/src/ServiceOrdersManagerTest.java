...
public class ServiceOrdersManagerTest {
	...
	private ServiceOrdersManager orders2;
	private final int DISPLAY_TYPE = 0;  //choose 0 to test JTable, 1 for JList, 2 for both
	...
	public void setUp() throws Exception {
		...
		//STUDENT
		orders2 = new ServiceOrdersManager();
		orders2.startService(order5);
		orders2.startService(order6);
		orders2.startService(order7); //order should be 6, 7, 5 by name
	}
	...
	public void testStartServiceOrderSTUDENT() {
		...
		Vector<String> vList;
		  ...
		  if (DISPLAY_TYPE==0 || DISPLAY_TYPE==2) {
			//verify initial contents of orders in order by owner
			table = orders2.listByKeyTable(2);
			assertEquals("Clancy, Robert", table[0][0]);
			assertEquals("Johnson, Danny", table[1][0]);
			assertEquals("Majors, James", table[2][0]);
		  }
		  if (DISPLAY_TYPE==1 || DISPLAY_TYPE==2) {
			//verify initial contents of orders in order by owner
			vList = orders2.listByKeyVector(2);
			assertEquals("Clancy Robert Volvo S40 1 no yes yes", vList.get(0));
			assertEquals("Johnson, Danny 7 Buick Regal yes no yes", vList.get(1));
			assertEquals("Majors James 5 Honda Civic yes no no", vList.get(2));
		  }
		  try{
			//add service order
			orders2.startService(order2);
			}
		  catch (Exception e)
			{
				fail("This should not have raised an exception");
		  ...
		  if (DISPLAY_TYPE==0 || DISPLAY_TYPE==2) {
			  ...
			  assertEquals("Clancy, Robert", table[0][0]);
			  assertEquals("Johnson, Danny", table[1][0]);
			  assertEquals("Jones, Erica", table[2][0]);
			  assertEquals("Majors, James", table[3][0]);
		  ...
		  if (DISPLAY_TYPE==1 || DISPLAY_TYPE==2) {
			//verify modified contents of orders in order by owner
			vList = orders2.listByKeyVector(2);
			assertEquals("Clancy Robert 1 Volvo S40 no yes yes", vList.get(0));
			assertEquals("Johnson, Danny 7 Buick Regal yes no yes", vList.get(1));
			assertEquals("Jones, Erica Ford Focus 2 yes yes no", vList.get(2));
			assertEquals("Majors James 5 Honda Civic yes no no", vList.get(3));
	...
	public void testFinishServiceSTUDENT() {
	    ...
	    if (DISPLAY_TYPE==1 || DISPLAY_TYPE==2) {
			//verify initial contents of orders in order by owner
			vList = orders2.listByKeyVector(2);
			assertEquals("Clancy Robert 6 Volvo S40  no yes yes", vList.get(0));
			assertEquals("Johnson, Danny 7 Buick Regal yes no yes", vList.get(1));
			assertEquals("Majors James 5 Honda Civic yes no no", vList.get(2));
		...
		if (DISPLAY_TYPE==0 || DISPLAY_TYPE==2) {
			...
			table = orders2.listByKeyTable(2);
			assertEquals("Clancy, Robert", table[0][0]);
			 assertEquals("Majors, James", table[1][0]);
		...
		if (DISPLAY_TYPE==1 || DISPLAY_TYPE==2) {
			...
			vList = orders2.listByKeyVector(2);
			assertEquals("Clancy Robert 6 Volvo S40  no yes yes", vList.get(0));
			assertEquals("Majors James 5 Honda Civic yes no no", vList.get(1));
		}

		//try to delete an order with invalid order #, should raise an exception
		try{
			//add service order
			orders.finishService(9);
		}
		catch (Exception e)
		{
			raised = true;
