 ...
 * The OneTimeApointment class holds information about a 
 * type of date,GregorianCalendar object and description of appointment 
 *  and computes whether given date is a one time occurrence for that specific date
 * @author E Cross
 *
 */
...
public class OneTimeAppointment extends Appointment  {
	 ...
	 */
	public OneTimeAppointment(char group, GregorianCalendar d, String describe){
	 ...
	 * @return success returns true if appt is one time occurrence
	 */
	public boolean Occurs_On(int day, int month, int year){
		...
		if(((newAppointment.get(Calendar.YEAR)) == today.get(Calendar.YEAR)) &&						 
		...
		else if(!(newAppointment.before(today) || newAppointment.after(today))){
			success = true;
		}
	...
	public String TimetoString(){
		GregorianCalendar thisDate = getDate();
		now = thisDate.getTime();
